...
export function foldEvents(entries = []) {
	...
	let turnStartTime = null; // 最近一次 turn/start 的时间戳（思考时长 / HUD ⏱ 用）
	let turnEndedAt = null; // 最近一次 turn/end 的事件时间戳（稳定的完成耗时）
	let lastSeq = -1;
	let model = null;
	...
	for (const entry of entries) {
		...
		const et = typeof event.time === "number" ? event.time : null;

		switch (event.type) {
			case "turn/start": {
				if (typeof data.turn === "number" && data.turn > turn) turn = data.turn;
				if (et) turnStartTime = et;
				break;
			}
			case "turn/end": {
				lastTurnEnd = { turn: data.turn ?? turn, reason: data.reason ?? null, seq: event.seq, time: et };
				if (et) turnEndedAt = et;
				break;
			}
			case "request/header": {
			...
			case "request/context": {
				// 类型标注：捕获路由的 contextWindow 用于 HUD 上下文占用百分比。
				if (data && typeof data.contextWindow === "number") contextWindow = data.contextWindow;
				break;
			}
			case "user/message": {
			...
			case "tool/result": {
				...
				if (!entry) {
					// 单独到达的 tool/result（同一批 fold 里无前置 tool/call）：仍产出「状态更新」工具项。
					// name/args 缺失，由 live 层的 mergeToolsByCallId 就地套到已有 running 卡上（Codex 评审 #4）。
					tools.push({
						seq: event.seq,
						callId,
						name: undefined,
				...
				if (et) entry.finishedAt = et;
				// 保留工具结果里可能的 diff meta（Claude Code /diff、dsh-tool-fs contextual diff）。
				entry.diffMeta = data.meta ?? undefined;
				break;
			}
			default:
	...
	return { messages, tools, turn, lastTurnEnd, turnStartTime, turnEndedAt, lastSeq, model, contextWindow };
}
