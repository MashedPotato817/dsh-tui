...
export const LOCAL_COMMANDS = {
	...
	status: { description: "当前会话状态（ID/模型/上下文/模式）" },
...
export function routeSlash(input) {
	...
	if (parsed.name === "status") return { local: true, action: "status" };
...
/** 合并 BUILTIN + LOCAL + 自定义，供 SlashPanel 展示。custom 优先覆盖同名。 */
export function allCommands(custom = []) {
	const cmds = [
		...BUILTIN_COMMANDS,
		...
		...custom.map((c) => ({
			name: c.name,
			description: c.description || `自定义命令（${c.source}）`,
			customBody: c.body
		}))
	];
	// name 去重：自定义 > local > builtin（后者追加在前，前者覆盖）
