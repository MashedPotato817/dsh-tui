...
import { dedupeUsage } from "./usage.js";
...
export function hudState({ view, session, prices, now }) {
	// 已落定消息按 seq 去重累加（防重复事件高估上下文/成本）；流式草稿单独加上。
	const usage = dedupeUsage(view.messages);
	if (view.streaming && view.streaming.usage) {
		const su = view.streaming.usage;
		usage.input += su.inputTokens ?? 0;
		usage.output += su.outputTokens ?? 0;
		usage.cacheRead += su.cacheReadTokens ?? 0;
		usage.cacheWrite += su.cacheWriteTokens ?? 0;
	}
	...
	const contextPct = view.contextWindow && view.contextWindow > 0
		...
		: null;
	const hasUsage = usage.input > 0 || usage.output > 0 || usage.cacheRead > 0;
	return {
		...
		hasUsage,
		contextPct,
		// 只有真正有 token 用量或价格时才显示成本（避免启动时裸 $0.0000）
		costUsd: hasUsage ? costUsd(usage, price) : null,
...
/**
 * token 计数紧凑格式化（Claude Code 心智）：<1000 → "123"；≥1000 → "1.2k"；≥1e6 → "1.5M"。
 * @param {number} n
 * @returns {string}
 */
