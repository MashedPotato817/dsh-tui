...
/**
 * 把文本按终端显示宽度切成真实可分页的视觉行。与 Ink 自动折行使用同一宽度心智，
 * 保留显式换行；用于视口边界消息的纯文本降级渲染，避免“一个超长段落只有一个
 * Markdown row，fromLine 一切就为空”的问题。
 */
export function splitVisualLines(text, columns) {
	...
	for (const logical of String(text ?? "").split("\n")) {
		...
		for (const ch of logical) {
			const chWidth = EAST_ASIAN_RE.test(ch) ? 2 : 1;
			if (line && used + chWidth > width) {
				out.push(line);
				line = "";
				used = 0;
			}
...
/** 截取一段真实视觉行并重新以换行连接。 */
export function sliceTextByVisualLines(text, columns, startLine = 0, maxLines = Infinity) {
	const lines = splitVisualLines(text, columns);
	const start = Math.max(0, startLine);
	const end = Number.isFinite(maxLines) ? start + Math.max(0, maxLines) : lines.length;
...
export function windowViewport(messages, budget, columns, scrollLines = 0) {
	if (!Array.isArray(messages) || messages.length === 0) {
		return { startMsg: 0, startLine: 0, scrollLines: 0, budget: Math.max(1, budget), columns };
	}
	...
	return { startMsg, startLine, scrollLines: clampedScroll, budget: Math.max(1, budget), columns };
}
