...
// ——— 有界缓存：已定稿消息的 Markdown 解析结果按文本缓存，防每秒 `now` 重绘时重复解析长正文。
// 纯函数/零依赖；LRU 有界避免内存无界增长。手动调用方（MarkdownBody）用 cachedParseMarkdown。
const MARKDOWN_CACHE_MAX = 64;
const cache = new Map();
export function cachedParseMarkdown(text) {
	const key = text;
	if (cache.has(key)) {
		// LRU：重新置后
...
export function markdownCacheSize() { return cache.size; }
export function clearMarkdownCache() { cache.clear(); }
