	...
	#syncStateMeta(view) {
		if (view.lastTurnEnd) {
			...
			if (newer && this.state.running) {
				...
				if (!this.state.turnEndedAt) this.state.turnEndedAt = Date.now();
			}
		}
	...
	#onFrame(frame, rpcId) {
		...
		switch (frame.type) {
			...
			case "question/requested": {
				...
				// 默认交互也无「完整交互式提问 UI」（policy.js 记录如此），故 question 统一走
				// #autoReply 的安全策略自动应答：普通问询选首个选项，plan-review 在 manual/plan
				// 档自动拒绝（decline）。保留 pendingQuestions 供 question/resolved 清理与未来
				// 交互式 UI 接管。approval 仍挂起等 y/Y/n（UI 有对应交互）。
