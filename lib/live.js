...
import { answerApproval, answerQuestions } from "./policy.js";
...
export class LiveConversation {
	 ...
	 */
	constructor({ client, session, stream, onState, policy = null }) {
		this.client = client;
		...
		switch (frame.type) {
			...
			case "approval/requested": {
				this.state.pendingApprovals = [
					...(this.state.pendingApprovals ?? []),
					{ approvalId: frame.approvalId, toolName: frame.toolName, rpcId }
				];
				this.#autoReply(frame, rpcId).catch((error) => {
	...
	async #autoReply(frame, rpcId) {
		...
		if (frame.type === "question/requested") {
			value = answerQuestions(this.session.sessionId, frame.questions, this.policy?.questions ?? {});
		} else if (frame.type === "approval/requested") {
			value = answerApproval(this.session.sessionId, frame, this.policy ?? {});
		} else {
			return;
		}
