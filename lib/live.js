...
export class LiveConversation {
	...
	async #historySync(intervalMs = 1200) {
		while (!this.ac.signal.aborted) {
			try {
				...
				if (!view.messages.length && !view.lastTurnEnd) {
	...
	#mergeView(view) {
		...
		if (freshTools.length) this.state.tools = [...this.state.tools, ...freshTools];
	...
	/** 同步 lastTurnEnd/turn/model/contextWindow/running。 */
	#syncStateMeta(view) {
		...
		switch (frame.type) {
			...
			case "approval/requested": {
				...
				if (presetAllowed) {
					...
					this.client.respond(rpcId, { ok: true, value }).catch(() => {});
					this.state.pendingApprovals = this.state.pendingApprovals.filter(
